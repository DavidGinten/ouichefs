 ...
 */
void display_block_summary(const struct ouichefs_block_display *display_data,
			   int file_slice)
{
...
int main(int argc, char *argv[])
{
	...
	if (argc != 2) {
		...
		fprintf(stderr,
			"Display the block content for a small file in ouichefs\n");
		return 1;
	}
	...
	if (ret < 0) {
		...
		case EINVAL:
			fprintf(stderr,
				"Error: File is not a regular file or exceeds 128 bytes or doesn't use slices\n");
			break;
		case ENODATA:
		...
		case ENOTTY:
			fprintf(stderr,
				"Error: IOCTL not supported (wrong filesystem?)\n");
			break;
		default:
	...
	printf("! is Unicode null symbol or an actual ! | ");
	printf("? is non-printable char or an actual ?\n\n");
