...
#define OUICHEFS_SMALL_FILE_THRESHOLD (OUICHEFS_SLICE_SIZE * OUICHEFS_DATA_SLICES)  /* ~4KB */
 ...
 */
static inline bool ouichefs_has_contiguous_slices(uint32_t bitmap, uint32_t start_slice, uint32_t count)
{
	if (start_slice == 0 || start_slice + count > OUICHEFS_SLICES_PER_BLOCK) {
		return false;
	}
...
bool ouichefs_has_contiguous_slices(uint32_t bitmap, uint32_t start_slice, uint32_t count);
