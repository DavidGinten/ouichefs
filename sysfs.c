...
/* Global kobject for ouichefs sysfs */
 ...
 * as we need to handle multiple partitions at the same time.
 */
#define OUICHEFS_SYSFS_HASH_BITS 4 /* 16 buckets should be enough */
 ...
 */
static int ouichefs_count_sliced_stats(struct super_block *sb,
				       uint32_t *sliced_blocks,
				       uint32_t *free_slices)
{
	struct ouichefs_sb_info *sbi = OUICHEFS_SB(sb);
	struct buffer_head *bh;
	struct ouichefs_sliced_block_meta *meta;
	uint32_t current_block;
	uint32_t total_sliced_blocks = 0;
	uint32_t total_free_slices = 0;
	...
	*free_slices = 0;
	...
	while (current_block != 0) {
		...
		if (le32_to_cpu(meta->magic) == OUICHEFS_SLICED_MAGIC) {
			...
			uint32_t free_slices_in_block = 0;
			int i;
			...
			for (i = 1; i < OUICHEFS_SLICES_PER_BLOCK; i++) {
				if (bitmap & (1U << i))
					free_slices_in_block++;
			}

			total_free_slices += free_slices_in_block;
		}
	...
	/* We also need to count fully occupied sliced blocks. */
	/* This requires scanning all data blocks to find sliced blocks not in the free list */
	uint32_t data_start = 1 + sbi->nr_istore_blocks + sbi->nr_ifree_blocks +
			      sbi->nr_bfree_blocks;
	uint32_t block_num;
	...
	for (block_num = data_start; block_num < sbi->nr_blocks; block_num++) {
		...
		if (!test_bit(block_num, sbi->bfree_bitmap)) {
			...
			if (bh) {
				meta = (struct ouichefs_sliced_block_meta *)
					       bh->b_data;
				if (le32_to_cpu(meta->magic) ==
				    OUICHEFS_SLICED_MAGIC) {
					uint32_t bitmap =
						le32_to_cpu(meta->slice_bitmap);

					/* If this block is not in the free list */
					/* (i.e. no free slices, bitmap == 0), count it */
					if (bitmap == 0)
						total_sliced_blocks++;
				}
				brelse(bh);
	...
	*sliced_blocks = total_sliced_blocks;
	*free_slices = total_free_slices;
	return 0;
}

/* Count files and collect size statistics by scanning all inodes */
static int ouichefs_count_file_stats(struct super_block *sb, uint32_t *files,
				     uint32_t *small_files,
				     uint64_t *total_data_size)
{
	struct ouichefs_sb_info *sbi = OUICHEFS_SB(sb);
	...
	for (i = 1; i < sbi->nr_inodes; i++) {
		...
		if (nlink == 0) {
			pr_debug(
				"Skipping deleted inode %u (nlink=0, mode=%u, size=%u)\n",
				i, mode, size);
			brelse(bh);
			continue;
		}
		...
		if (mode == 0) {
			pr_debug(
				"Skipping uninitialized inode %u (mode=0, nlink=%u, size=%u)\n",
				i, nlink, size);
			brelse(bh);
			continue;
		}
		...
		if (S_ISREG(mode)) {
			...
			if (size > 0 && size <= OUICHEFS_DATA_SLICES *
							OUICHEFS_SLICE_SIZE) {
				total_small++;
				pr_debug(
					"Found small file: inode %u, size %u bytes, nlink=%u\n",
					i, size, nlink);
			} else if (size > 128) {
				pr_debug(
					"Found large file: inode %u, size %u bytes, nlink=%u\n",
					i, size, nlink);
			} else if (size == 0) {
				pr_debug(
					"Found empty file: inode %u, nlink=%u\n",
					i, nlink);
				/* Empty files are regular files but not small files */
			}
		} else {
			pr_debug(
				"Skipping non-regular file: inode %u, mode=0%o, nlink=%u\n",
				i, mode, nlink);
		}
	...
	*total_data_size = total_size;
	...
	pr_debug(
		"File stats scan result: total=%u, small=%u, total_size=%llu\n",
				  ...
				  struct ouichefs_stats *stats)
{
	...
	/* Calculate total_data_blocks and used_blocks */
	uint32_t total_data_blocks =
		sbi->nr_blocks - 1 - sbi->nr_istore_blocks -
		sbi->nr_ifree_blocks - sbi->nr_bfree_blocks;
	stats->used_blocks = total_data_blocks - sbi->nr_free_blocks;

	/* Sliced block statistics */
	ret = ouichefs_count_sliced_stats(sb, &stats->sliced_blocks,
					  &stats->total_free_slices);
	if (ret)
		return ret;

	/* File statistics */
	ret = ouichefs_count_file_stats(sb, &stats->files, &stats->small_files,
					&stats->total_data_size);
	if (ret)
		return ret;
	...
	stats->total_used_size =
		(uint64_t)stats->used_blocks * OUICHEFS_BLOCK_SIZE;

	/* Calculate efficiency (percentage) */
	if (stats->total_used_size > 0) {
		stats->efficiency = (uint32_t)((stats->total_data_size * 100) /
					       stats->total_used_size);
	} else {
		stats->efficiency = 0;
	}
	...
	pr_debug(
		"Stats: used_blocks=%u, sliced_blocks=%u, files=%u, small_files=%u\n",
		stats->used_blocks, stats->sliced_blocks, stats->files,
		stats->small_files);
				    ...
				    const char *partition_name)
{
	...
	entry->sb = sb;
	strncpy(entry->device_name, partition_name,
...
/*
 * Remove partition-specific sysfs directory on filesystem unmount
 */
...
/*
 * Cleanup sysfs interface (i.e. all sysfs entries) when the kernel module is unloaded
 */
